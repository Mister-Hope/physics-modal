...
/**
 * Creates a high-resolution procedural canvas texture for the fixed sleeve (固定套管主尺).
 * Features:
 * - Satin chrome metallic background with horizontal brushed finish
 * - Central horizontal datum line (基准线) exactly in the middle
 * - Upper ticks: integer millimeters (0, 1, 2, ... 25 mm) with bold labels every 5 mm
 * - Lower ticks: half millimeters (0.5, 1.5, ... 24.5 mm)
 * - Ticks and numbers are oriented along the sleeve length without angular wrapping
 */
export function createSleeveTexture(): {
  ...
  const mmSpacing = (endX - startX) / 25.0; // 68 pixels per mm
  ...
  for (let mm = 0; mm <= 25; mm++) {
    ...
    if (isFive) {
      ctx.fillText(mm.toString(), x, datumY - tickLen - 12);
    }
...
/**
 * Creates procedural canvas texture for the movable thimble bevel (微分筒锥形读数圈).
 * - Circumference (U): 50 divisions (0 to 49)
 * - Length (V): 0 is the left measuring edge, 1 is the right edge meeting the body.
 * - Division tick lines start at the LEFT edge and extend to the right.
 * - Numbers (0, 5, 10, ... 45) are clearly legible and properly proportioned without squishing.
 */
export function createThimbleBevelTexture(): THREE.CanvasTexture {
  ...
  const divSpacing = width / 50.0; // 40.96 px per division
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (isFive) {
      ...
      const aspectCorrection = (512.0 / 4.0) / (2048.0 / 33.6); // ~2.10
      ctx.scale(aspectCorrection, 1.0);
...
/**
 * Creates frame inscription texture ("0-25mm  0.01mm").
 * Brushed satin bright silver metallic face with precision laser-engraved typography.
 */
export function createFrameLabelTexture(): THREE.CanvasTexture {
  const width = 1024;
  const height = 256;
  const canvas = document.createElement('canvas');
  canvas.width = width;
 ...
 */
export function createRatchetKnurlTexture(): THREE.CanvasTexture {
  const width = 512;
  const height = 256;
  const canvas = document.createElement('canvas');
  canvas.width = width;
